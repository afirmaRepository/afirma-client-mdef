...
final class SignDataPanel extends JPanel {
	...
	void createUI(final File signFile, final byte[] sign, final JComponent fileTypeIcon, final X509Certificate cert,
		...
		filePath.setText(
				signFile == null ? SimpleAfirmaMessages.getString("SignDataPanel.24") : signFile.getAbsolutePath()); //$NON-NLS-1$
		filePath.addMouseListener(new MouseAdapter() {
			@Override
			public void mousePressed(final MouseEvent me) {
				// me.isPopupTrigger() depende del Look & Feel y no se puede
				// usar
				if (me.getButton() == MouseEvent.BUTTON3 && me.getClickCount() == 1) {
					new CopyMenuItem(filePath, SimpleAfirmaMessages.getString("SignDataPanel.30")) //$NON-NLS-1$
		...
		if (isOpennable && signFile != null) {
			...
			openFileButton.getAccessibleContext()
					.setAccessibleDescription(SimpleAfirmaMessages.getString("SignDataPanel.6")); //$NON-NLS-1$
			openFileButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(final ActionEvent ae) {
					try {
						...
						LOGGER.warning("Error abriendo el fichero con el visor por defecto: " + e //$NON-NLS-1$
						);
						AOUIFactory.showErrorMessage(SignDataPanel.this,
		...
		if (cert != null) {
			...
			if (certInfo != null) {
				...
				// Para que se detecten apropiadamente los hipervinculos hay que
				// establecer
				...
				this.certDescription.getAccessibleContext()
						.setAccessibleName(SimpleAfirmaMessages.getString("SignDataPanel.13")); //$NON-NLS-1$
				this.certDescription.getAccessibleContext()
						.setAccessibleDescription(SimpleAfirmaMessages.getString("SignDataPanel.14")); //$NON-NLS-1$

				final EditorFocusManager editorFocusManager = new EditorFocusManager(this.certDescription,
				...
				if (cfv != null) {
					...
					this.validateCertButton.getAccessibleContext()
							.setAccessibleName(SimpleAfirmaMessages.getString("SignDataPanel.17")); //$NON-NLS-1$
					this.validateCertButton.getAccessibleContext()
						...
						public void actionPerformed(final ActionEvent ae) {
							...
							case VALID:
								if(!PreferencesManager.getBoolean(PREFERENCE_CHECK_CERTIFICATE_PSSDEF, false)){
									...
									validationMessageType = JOptionPane.INFORMATION_MESSAGE;
									validationMessage = SimpleAfirmaMessages.getString("SignDataPanel.19");//quitar
									try {
										validationMessage = validaCertificado.validaCert(cert);
										if(validationMessage.trim().equals(PssdefConstants.certOK)){
											validationMessageType = JOptionPane.INFORMATION_MESSAGE;
											//para quitar
											//throw new PssdefValidaException("");
											//para quitar
										}else{
											validationMessageType = JOptionPane.ERROR_MESSAGE;
										}
									} catch (PssdefValidaException e) {
										if(PreferencesManager.get(PREFERENCE_CN_CA_CERT_SERVICE, "").contains(
												AOUtil.getCN(cert.getIssuerX500Principal().getName()))){
											
											MdefCertificateVerifier mdefCertificateVerifier = new MdefCertificateVerifier();
											try {
												mdefCertificateVerifier.setP(ExtraParamsHelper.loadValidationExtraParams());
												mdefCertificateVerifier.verifyRevocation(cert).check();
												ValidationResult validationResult = mdefCertificateVerifier.verifyRevocation(cert);
												...
												LOGGER.severe(e.getMessage()+e1);
												validationMessage = e1.getMessage();
							...
							AOUIFactory.showMessageDialog(SignDataPanel.this, validationMessage,
		...
		// En Apple siempre hay barras, y es el SO el que las pinta o no depende
		// de si hacen falta
	 ...
	 * @throws IOException
	 *             Si ocurren problemas relacionados con la lectura de los datos
	 */
	private static CompleteSignInfo getSignInfo(final byte[] signData) throws IOException {
		...
		try {
			signInfo.setTimestampsInfo(TimestampsAnalyzer.getTimestamps(signData));
		} catch (final Exception e) {
	...
	private static JTree getSignDataTree(final CompleteSignInfo signInfo, final KeyListener extKeyListener,
		...
		if (signInfo.getData() == null) {
			...
			dataInfoBranch.add(new DefaultMutableTreeNode(
					new ShowFileLinkAction(SimpleAfirmaMessages.getString("SignDataPanel.28"), //$NON-NLS-1$
							signInfo.getData())));
		}
...
//        }
		...
		if (signInfo.getTimestampsInfo() != null && !signInfo.getTimestampsInfo().isEmpty()) {
			final TreeModelManager treeManagerTimestamps = new TreeModelManager(signInfo.getTimestampsTree());
			final DefaultMutableTreeNode timestampsBranch = treeManagerTimestamps.getSwingTree();
			timestampsBranch.setUserObject(SimpleAfirmaMessages.getString("SignDataPanel.42") //$NON-NLS-1$
			);
			root.add(timestampsBranch);
		}

		// final DefaultTreeCellRenderer treeRenderer = new
		// DefaultTreeCellRenderer();
		final LinksTreeCellRenderer treeRenderer = new LinksTreeCellRenderer();
		treeRenderer.setLeafIcon(null);
		treeRenderer.setClosedIcon(
				Platform.OS.WINDOWS.equals(Platform.getOS()) || Platform.OS.MACOSX.equals(Platform.getOS()) ? null
						: UIManager.getDefaults().getIcon("TREE.collapsedIcon")); //$NON-NLS-1$
		treeRenderer
				.setOpenIcon(Platform.OS.WINDOWS.equals(Platform.getOS()) || Platform.OS.MACOSX.equals(Platform.getOS())
						? null : UIManager.getDefaults().getIcon("TREE.expandedIcon")); //$NON-NLS-1$
