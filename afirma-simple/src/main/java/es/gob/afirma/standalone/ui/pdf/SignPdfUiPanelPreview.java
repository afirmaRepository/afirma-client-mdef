...
public final class SignPdfUiPanelPreview extends JPanel implements KeyListener {
	...
	SpinnerNumberModel sizeSpinnerModel = new SpinnerNumberModel(INIT_TEXT_SIZE, MIN_TEXT_SIZE, MAX_TEXT_SIZE,
			STEP_TEXT_SIZE);
	...
	SignPdfUiPanelPreview(final SignPdfUiPanelListener spul, final Properties p, final BufferedImage im) {

		if (spul == null) {
			throw new IllegalArgumentException("La clase a la que notificar la seleccion no puede ser nula" //$NON-NLS-1$
			);
		}
		this.listener = spul;
		this.prop = p;
	...
	void createUI() {
		...
		getAccessibleContext().setAccessibleDescription(SignPdfUiMessages.getString("SignPdfUiPreview.3") //$NON-NLS-1$
		);
	...
	private JPanel createPreviewPanel() {
		...
		panel.setBorder(BorderFactory.createTitledBorder(SignPdfUiMessages.getString("SignPdfUiPreview.0")) //$NON-NLS-1$
		);
					...
					if (transData instanceof List) {
						...
						filename.startsWith("ftp://") //$NON-NLS-1$
							...
							try {
								...
								LOGGER.warning(
										"Ha fallado la operacion de arrastrar y soltar al obtener la ruta del fichero arrastrado: " //$NON-NLS-1$
												+ e);
								dtde.dropComplete(false);
								return;
							}
						...
						try {
							...
							LOGGER.warning(
									"Ha fallado la operacion de arrastrar y soltar al cargar el fichero arrastrado: " //$NON-NLS-1$
											+ e);
							dtde.dropComplete(false);
						}
					}
		...
		this.viewLabel.getAccessibleContext().setAccessibleDescription(SignPdfUiMessages.getString("SignPdfUiPreview.9") //$NON-NLS-1$
		);
			...
			public void mouseClicked(final MouseEvent evt) {
				if (SwingUtilities.isRightMouseButton(evt)) {
					...
					try {
						...
						AOUIFactory.showMessageDialog(SignPdfUiPanelPreview.this,
	...
	private JPanel createFontPanel() {
		...
		panel.setBorder(BorderFactory.createTitledBorder(SignPdfUiMessages.getString("SignPdfUiPreview.1")) //$NON-NLS-1$
		);

		this.letterType = new JComboBox<>(FontResource.getAllFontresources());
		setViewFont(new Font(((FontResource) this.letterType.getSelectedItem()).getFont().getFontName(), getStyle(),
				getSelectedSize()));
		...
		this.padesSignReason.getDocument().addDocumentListener(new DocumentListener() {
			@Override
		...
		this.padesSignProductionCity.getDocument().addDocumentListener(new DocumentListener() {
			@Override
			public void removeUpdate(final DocumentEvent e) {
				ComposedTex();
				showPreview();
...
////		
	...
	private JPanel createTextAreaPanel() {
		...
		final JScrollPane scrollPane = new JScrollPane(this.textArea, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
	 ...
	 * @throws IOException
	 *             Si hay problemas en el pintado de la imagen.
	 */
	void loadFile(final File file) throws IOException {
		...
		if (errorMessage != null) {
			AOUIFactory.showErrorMessage(SignPdfUiPanelPreview.this, errorMessage,
					SimpleAfirmaMessages.getString("SimpleAfirma.7"), //$NON-NLS-1$
					JOptionPane.ERROR_MESSAGE);
	...
	private void createImage() {
		final BufferedImage bi = new BufferedImage(this.image.getWidth(), this.image.getHeight(),
	...
	void paintImage(final String path) throws IOException {
		...
		final BufferedImage newImage = new BufferedImage(this.image.getWidth(), this.image.getHeight(),
				this.image.getType());

		final double imageScale = determineImageScale(bi.getWidth(), bi.getHeight(), this.image.getWidth(),
				this.image.getHeight());

		final Graphics2D g = newImage.createGraphics();
		g.drawImage(this.image, 0, 0, null);
		g.drawImage(bi.getScaledInstance((int) (bi.getWidth() * imageScale), (int) (bi.getHeight() * imageScale),
	...
	private static double determineImageScale(final int sourceWidth, final int sourceHeight, final int targetWidth,
	...
	void showPreview() {
		final BufferedImage bi = new BufferedImage(this.image.getWidth(), this.image.getHeight(), this.image.getType());
	...
	private void resizeImage(final JPanel panel) {
		...
		final BufferedImage newBi = new BufferedImage(newWidth, newHeight, this.image.getType());
	...
	private final class ComboRenderer extends BasicComboBoxRenderer {
		...
		public Component getListCellRendererComponent(final JList list, final Object value, final int index,
			...
			final JLabel renderer = (JLabel) this.defaultRenderer.getListCellRendererComponent(list, value, this.row,
	...
	void saveRubric() {
		try {
			...
			//De existir el error se solicita al usuario continuar o no
			if (ErrorMaxValueExceded()) {
				//si el usuario no desea continuar se lanza la excepción para quedarse en la misma pantalla
				throw new AOCancelledOperationException("Se ha reusado el continuar con la firma"); //$NON-NLS-1$
			}
